...
describe("dispatchPreparedSlackMessage preview fallback", () => {
  ...
  it.each([
    ...
    "keeps compact progress authored-only (style=%s, native=%s)",
    async (style, native) => {
      const draftStream = createDraftStreamStub();
      createSlackDraftStreamMock.mockReturnValueOnce(draftStream);
      finalizeSlackPreviewEditMock.mockResolvedValueOnce(undefined);
      ...
      // A quiet draft keeps authored reasoning and the failed command; routine
      // tool rows and plan checklists stay out.
