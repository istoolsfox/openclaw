...
final class WatchRealtimeTransport: @unchecked Sendable {
    ...
    private func discoverFlows() throws {
        ...
        let locals = try Self.localAddresses()
        let bindings = locals.flatMap { source in
            remotes.filter { Self.sameFamily(source, $0) }.map {
                Binding.discover(source: source, destination: $0)
            }
        }
